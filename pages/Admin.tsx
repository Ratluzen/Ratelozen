...
const Admin: React.FC<Props> = ({ 
  ...
  const analytics = useMemo(() => {
    ...
    const salesChart = last7Days.map(date => {
        ...
        const dailyOrders = orders.filter(o => {
            return o.status === 'completed' && o.date.startsWith(dateStr);
        });
  ...
  const recentOrders = orders.slice(0, 5).map(o => ({
      ...
      price: `$${o.amount}`,
      status: o.status,
      time: o.date.split(',')[1] // Simple time extraction
  }));
  ...
  const handleConfirmCancel = () => {
      ...
      // 2. REFUND Logic: Add amount back to user's wallet
  ...
  const handleSaveProduct = () => {
    ...
    const productToSave: Product = editingProduct 
      ? { ...editingProduct, ...prodForm } as Product
      : {
    ...
    if (productToSave.autoDeliverStock) {
        let tempInventory = [...inventory];
        let ordersUpdated = false;

        const updatedOrders = orders.map(order => {
            if (order.status === 'pending' && order.productName === productToSave.name) {
                const regionId = productToSave.regions?.find(r => r.name === order.regionName)?.id;
                const denomId = productToSave.denominations?.find(d => d.label === order.quantityLabel)?.id;

                const stockIndex = tempInventory.findIndex(item =>
                    item.productId === productToSave.id &&
                    !item.isUsed &&
  ...
  const handleUpdateBalance = (type: 'add' | 'deduct') => {
    ...
    // Log Transaction for manual Admin adjustment
    const newTx: Transaction = {
        id: `admin-${Date.now()}`,
        title: type === 'add' ? 'إيداع من الإدارة' : 'خصم من الإدارة',
        date: new Date().toLocaleString('en-US'),
        amount: val,
        type: type === 'add' ? 'credit' : 'debit',
        status: 'completed',
        icon: type === 'add' ? Plus : Trash2
    };
              ...
              <div className={`bg-[#242636] p-5 rounded-2xl border border-gray-700 shadow-lg relative ${openInvDropdown ? 'z-[50]' : 'z-20'}`}>
                  ...
                  {invSelectedProduct && (
                      <div className="flex flex-col gap-5 mb-5 relative z-[48]">
                                ...
                                {openInvDropdown === 'region' && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-[#1f212e] border border-gray-700 rounded-xl shadow-2xl z-[60] p-2 animate-fadeIn grid grid-cols-2 gap-2">
                                        <button 
                                            ...
                                            onClick={(e) => { e.stopPropagation(); setInvSelectedRegion(''); setOpenInvDropdown(null); }}
                                        >
                                            -- الكل --
                                        </button>
                                        {selectedProductObj?.regions?.map(r => (
                                            <button 
                                                ...
                                                onClick={(e) => { e.stopPropagation(); setInvSelectedRegion(r.id); setOpenInvDropdown(null); }}
                                            >
                          ...
                          {(selectedProductObj?.denominations?.length ?? 0) > 0 && (
                             <div className="flex-1 relative">
                                ...
                                >
                                    <span className="font-bold text-sm">{selectedProductObj?.denominations?.find(d => d.id === invSelectedDenom)?.label || '-- الكل --'}</span>
                                ...
                                {openInvDropdown === 'denom' && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-[#1f212e] border border-gray-700 rounded-xl shadow-2xl z-[60] p-2 animate-fadeIn grid grid-cols-2 gap-2">
                                        ...
                                        {selectedProductObj?.denominations?.map(d => (
                                            <button 
                                                key={d.id}
                                                type="button"
                  ...
                  <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                      {getFilteredInventory().length === 0 ? (
                          ...
                          getFilteredInventory().map(item => {
                              ...
                              const denom = prod?.denominations?.find(d => d.id === item.denominationId);
        ...
        {activeTab === 'terms' && (
          <div className="space-y-6">
             <div className="bg-[#242636] p-5 rounded-2xl border border-gray-700">
                ...
                <button 
                    className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-4 rounded-xl font-bold mt-6 shadow-lg transition-colors flex items-center justify-center gap-2" 
                    onClick={() => alert('تم حفظ الشروط بنجاح ✅')}
                >
                 ...
                 {activeProductTab === 'basic' && (
                     <div className="space-y-4 animate-fadeIn">
                        ...
                        <div className="flex gap-4">
                            <div className="flex-1">
                                <label className="text-xs text-gray-400 mb-1.5 block font-bold">السعر الأساسي ($)</label>
                                <div className="relative">
                                    <input type="number" className="w-full bg-[#13141f] p-3 rounded-xl border border-gray-700 text-white focus:border-yellow-400 outline-none transition-colors pl-8" value={prodForm.price} onChange={e => setProdForm({...prodForm, price: parseFloat(e.target.value)})} />
                     ...
                     <div className="space-y-6 animate-fadeIn">
                        ...
                        <div className="bg-[#242636] p-4 rounded-xl border border-gray-700">
                            ...
                            <div className="flex gap-2">
                                ...
                                <input className="flex-1 bg-[#13141f] p-2.5 rounded-xl border border-gray-700 text-white text-xs outline-none focus:border-yellow-400 transition-colors" type="number" placeholder="السعر" value={tempDenomPrice} onChange={e => setTempDenomPrice(e.target.value)} />
