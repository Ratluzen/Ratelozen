...
import { 
  ...
  Send, ShieldAlert, ChevronDown, AlertTriangle, Lock, Eye, EyeOff, Key, Copy
} from 'lucide-react';
...
const Profile: React.FC<Props> = ({ setView, currentCurrency, onCurrencyChange, terms, user, currencies, rateAppLink, onLogout, onUpdateUser }) => {
  ...
  const handleSavePassword = () => {
      ...
      // 3. Validation: Old Password (if user has one)
      if (user.password) {
          if (!passwordForm.oldPassword) {
             alert("يرجى إدخال كلمة المرور القديمة");
             return;
          }
  ...
  return (
    <div className="min-h-screen pb-24 bg-[#13141f] relative pt-6">
      ...
      <div 
        ...
        className="w-full px-4 mb-8 flex items-center gap-4 text-right group transition-transform active:scale-98 outline-none cursor-pointer select-none"
        role="button"
        tabIndex={0}
      >
         ...
         <div className="flex-1 flex flex-col items-end">
            ...
            <button 
                ...
                className="flex items-center gap-1.5 mt-1 cursor-pointer bg-[#242636] px-2 py-1 rounded-md border border-gray-800 hover:border-yellow-400/50 transition-colors group/id z-10"
                title="اضغط للنسخ"
            >
                {copiedId ? (
                    <Check size={12} className="text-green-500 animate-fadeIn" />
                ) : (
                    <Copy size={10} className="text-gray-600 group-hover/id:text-yellow-400" />
                )}
                <p className={`text-xs font-bold dir-ltr font-mono transition-colors ${copiedId ? 'text-green-500' : 'text-gray-500 group-hover/id:text-white'}`}>
                    ID: {user?.id || '---'}
                </p>
            </button>
         </div>
           ...
           <div className="fixed inset-0 z-[70] bg-[#13141f] animate-fadeIn flex flex-col">
               ...
               <div className="p-6">
                   <div className="flex flex-col items-center mb-8">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">{user?.password ? 'تغيير كلمة المرور' : 'تعيين كلمة مرور جديدة'}</h3>
